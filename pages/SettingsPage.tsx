...
const SettingsPage: React.FC = () => {
  ...
  const handleInputChange = (
    ...
    if (name === 'automationInterval') {
      const parsed = parseInt(value, 10);
      updateSettings({ automationInterval: isNaN(parsed) ? settings.automationInterval : parsed });
    } else {
  ...
  const currentOpacity = settings.watermarkDefaults.opacity ?? 0.7;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select
                ...
                value={String(settings.automationInterval)}
                onChange={handleInputChange}
                className="w-full p-2 bg-slate-800/50 border border-slate-700 rounded-lg"
              >
                <option value={String(60000 * 5)}>5 Minutes (for testing)</option>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input
                type="text"
                name="text"
                value={settings.watermarkDefaults.text || ''}
            ...
            <div>
              ...
              <select
                name="font"
                value={settings.watermarkDefaults.font}
                onChange={handleWatermarkChange}
                className="w-full p-2 bg-slate-800/50 border border-slate-700 rounded-lg"
              >
                <option>Inter</option>
            ...
            <div>
              ...
              <input
                type="color"
                name="color"
                value={settings.watermarkDefaults.color}
                onChange={handleWatermarkChange}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">
                Default Opacity: {Math.round(currentOpacity * 100)}%
              </label>
              <input
                type="range"
                name="opacity"

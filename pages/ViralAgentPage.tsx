...
import { MagicWandIcon, TrendingUpIcon, GoogleIcon, YouTubeIcon, TikTokIcon } from '../components/Icons';
...
const ContentIdeaCard: React.FC<ContentIdeaCardProps> = ({ idea }) => {
    ...
    const handleCreateBlog = async () => {
        ...
        try {
            ...
            // If generation fails, we should stop showing the loading state
    ...
    >
        <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm font-semibold text-slate-400">{idea.source}</span>
        </div>
        <h3 className="text-xl font-bold text-white mb-3 flex-grow group-hover:text-cyan-400 transition-colors">{idea.title}</h3>
        ...
        <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
                {idea.keywords.map(keyword => (
                    <span key={keyword} className="bg-cyan-900/50 text-cyan-300 text-xs font-medium px-2.5 py-1 rounded-full">{keyword}</span>
                ))}
            </div>
        </div>
        <div className="mt-auto pt-4 border-t border-white/10 flex items-center gap-3">
            ...
            >
                {isGeneratingBlog ? <><Loader/> Generating...</> : 'Create Blog Post'}
            </button>
            <button onClick={() => generateVideoFromIdea(idea)} className="w-full bg-slate-700/80 text-white font-semibold py-2 px-4 rounded-lg hover:bg-slate-700 transition-colors text-sm">
                Generate Video
            </button>
        </div>
    </motion.div>
...
const ViralAgentPage: React.FC = () => {
    ...
    const [selectedPlatforms, setSelectedPlatforms] = useState<string[]>(['google', 'youtube']);
    ...
    useEffect(() => {
        try {
            const savedStateJSON = localStorage.getItem(AGENT_STATE_KEY);
            if (savedStateJSON) {
                const savedState = JSON.parse(savedStateJSON);
                // Only set niche if it's not already set by settings
    ...
    const handleRunAgent = async () => {
        ...
        setError(null);
        try {
            const agentResults = await runViralAgent(niche, selectedPlatforms, setProgressMessage);
            setResults(agentResults);
            try {
                const stateToSave = { niche, results: agentResults };
                localStorage.setItem(AGENT_STATE_KEY, JSON.stringify(stateToSave));
            } catch (e) {
    ...
    return (
        <div className="max-w-7xl mx-auto">
            <div className="text-center mb-10">
                <h2 className="text-4xl md:text-5xl font-extrabold text-white mb-3 font-oswald text-gradient-cyan-sanguine">Viral Content Agent</h2>
                <p className="text-lg text-slate-400">Let AI find trending topics and generate your next viral hit.</p>
            </div>
            ...
            <div className="bg-slate-900/40 backdrop-blur-lg p-6 md:p-8 rounded-2xl shadow-2xl border border-white/10 mb-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2">
                        <label htmlFor="niche-input" className="block text-sm font-medium text-slate-300 mb-2">What is your content niche?</label>
                        <input 
                            type="text" 
                            id="niche-input"
                            value={niche}
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-300 mb-2">Analyze which platforms?</label>
                        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-2 gap-2">
                            {platforms.map(platform => (
                                <button
                                    ...
                                    disabled={isLoading}
                                    className={`p-2 w-full text-sm font-semibold rounded-md transition-colors disabled:opacity-50 ${selectedPlatforms.includes(platform.id) ? 'bg-cyan-500/20 text-cyan-300 ring-1 ring-cyan-500' : 'bg-slate-700/50 text-slate-300 hover:bg-slate-700'}`}
                                >
                                    {platform.name}
                                </button>
                            ))}
            ...
            <AnimatePresence>
                {isLoading && (
                    ...
                    >
                        <h3 className="text-2xl font-bold text-white mb-4">Analyzing Trends...</h3>
                        <p className="text-cyan-300 font-mono animate-pulse">{progressMessage}</p>
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <AnimatePresence>
                {results.length > 0 && (
                    ...
                    >
                         <h3 className="text-3xl font-bold text-white mb-6 text-center font-oswald">Content Ideas</h3>
                         <motion.div 

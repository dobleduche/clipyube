...
interface HealthResponse {
  status: 'ok' | string; // Adjust based on actual API response
}
...
const SmokeTestPage: React.FC = () => {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      continueTests = false; // Optional: Allow continuation
    }
    ...
    if (continueTests) {
      ...
      try {
        ...
        if (!apiResponse.ok) {
          ...
          if (errorData?.error) errorMsg = `Gemini API proxy test failed: ${errorData.error}`;
  ...
  const getStatusIndicator = (status: TestStatus) => {
    switch (status) {
      case 'pass':
        return <div className="w-4 h-4 rounded-full bg-green-500" title="Pass"></div>;
      case 'fail':
        return <div className="w-4 h-4 rounded-full bg-red-500" title="Fail"></div>;
      case 'running':
        return <Loader />;
      case 'pending':
      default:
        return <div className="w-4 h-4 rounded-full bg-slate-600" title="Pending"></div>;
    }
  };

...
type Agent = { id:string; name:string; model:string; status:AgentStatus; task:string; progress:number; };
...
const initialAgents: Agent[] = [
    ...
    // Adding placeholder agents for a complete looking pipeline
...
const ClipYubePage: React.FC = () => {
  ...
  const [urlInput, setUrlInput] = useState("https://test-videos.co.uk/vids/bigbuckbunny/mp4/h264/360/Big_Buck_Bunny_360_10s_1MB.mp4");
  const [error, setError] = useState<string | null>(null);

  const overall = useMemo(()=>Math.round(agents.reduce((s,a)=>s+a.progress,0)/agents.length),[agents]);

  const updateAgentStatus = useCallback((agentName: string, status: AgentStatus, progress?: number) => {
    setAgents(prev => prev.map(agent => {
        if (agent.name.toLowerCase() === agentName.toLowerCase()) {
            const newProgress = progress !== undefined ? progress : (status === 'done' ? 100 : agent.progress);
  ...
  const handleStartPipeline = async () => {
    ...
    setError(null);
    setAgents(initialAgents); // Reset state
    setSegments([]);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errData.error || 'Failed to submit URL to the pipeline.');
        }
    } catch (e) {
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto text-white">
      <div className="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
        <h1 className="text-2xl md:text-3xl font-bold font-oswald text-gradient-cyan-sanguine text-center sm:text-left">ClipYube – Viral Clip Engine</h1>
      ...
      <div className="bg-slate-900/40 p-4 rounded-2xl mb-6">
        <div className="flex flex-col md:flex-row gap-3">
          <input 
            ...
            className="flex-grow p-3 bg-slate-800/50 border border-slate-700 rounded-lg text-slate-200 placeholder-slate-500 focus:ring-2 focus:ring-cyan-500"
            disabled={isPipelineRunning}
          />
          <button 
            onClick={handleStartPipeline}
            disabled={isPipelineRunning || !urlInput.trim()}
            className="flex items-center justify-center gap-2 bg-cyan-600/80 text-white font-bold py-3 px-6 rounded-lg hover:bg-cyan-600 disabled:bg-slate-600 disabled:cursor-wait transition-colors"
          >
            {isPipelineRunning ? <><Loader /> Processing...</> : <><PlayIcon className="h-5 w-5"/> Start Pipeline</>}
          </button>
        </div>
      ...
      <div className="mt-8 space-y-6">
        <BuildGate percent={overall} onUpgrade={handleUpgrade} isRedirecting={isRedirecting}>
          ...
          <div className="rounded-2xl p-6 bg-slate-900/40 border border-white/10 mt-4">
            ...
            <div className="h-60 rounded-xl bg-black/40 grid place-items-center text-white/60">
              {overall >= 100 ? "Your clip is ready!" : "Rendered clip preview will appear here…"}
            </div>
          </div>
        </BuildGate>
      </div>

...
import { redisConnection as connection, transcodeQueue, queuesReady } from '../queues.js';
import Ffmpeg from "fluent-ffmpeg";
import path from "node:path";
import os from "node:os";
import fs from "node:fs/promises"; // Use promises API

if (!queuesReady || !connection || !transcodeQueue) {
  console.warn('[TranscodeWorker] Skipping initialization because Redis queues are not ready.');
} else {
  ...
  new Worker<TranscodeJobData, void, string>(transcodeQueueInstance.name, async (job) => {
